...
After some time realziation: 
The main thing is that , the it is a "function call" that is being returned so it will eval the params of the function . 

It was very simple and nothing to do with ternary operator . 

This works fine :
(Given in later part of book)
...
It would be a issue if it was a function that is being returned . 

Basically these sytax are spl and allow for val of branch only when needed.